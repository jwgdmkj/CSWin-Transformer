...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0.,
        ...
        if idx == -1:
            ...
            H_sp, W_sp = self.split_size, self.split_size * self.pos
        elif idx == 1:
            H_sp, W_sp = self.split_size * self.pos, self.split_size
        else:
            # print ("ERROR MODE", idx)
            exit(0)

        # print('----- LePEAttn var -----')
        # print(H_sp, W_sp, self.resolution, self.split_size)
        ...
        # print('idx ----------------', idx)
        # print('Hsp and Wsp', self.H_sp, self.W_sp)
        self.attn_drop = nn.Dropout(attn_drop)

    def im2cswin(self, x):
        '''
        x shape : [16, 3136(H*W), 32] = B, H*W, C
        H = W = 56 = sqrt(3136)

        --> x.shape : [16, 32, 56, 56] by x = img2windows(...)
        '''
        ...
        # print('im2cswin 3 : ', x.shape, ' because HSP and WSP is ', self.H_sp, self.W_sp)
    ...
    def forward(self, qkv):
        ...
        """
        # print('-------------------------------- 3) LePE Attn -------------------------------------- ')
        # print('0) input : qkv shape and H, W, head : ', qkv.shape, self.resolution, self.num_heads)
        ...
        q_ = rearrange(q_, 'b (h p1) (w p2) c -> (b h w) (p1 p2) c', p1=self.split_size, p2=self.split_size)
        # print('1) q_ k_ v_ - ', q_.shape, k_.shape, v_.shape)
        ...
        # print('2) q k v_ - ', q_.shape, k_.shape, v_.shape)

        # idx == 0 -> x_axis / 1 -> y_axis
        if self.idx == 0:
            # print('-----------x axis-------------')
            k_ = F.pad(k_, (self.split_size * self.pos, self.split_size * self.pos, 0, 0))
            k_ = F.unfold(k_, kernel_size=(self.split_size, self.split_size * (self.pos * 2 + 1)),
                          stride=self.split_size)
            v_ = F.pad(v_, (self.split_size * self.pos, self.split_size * self.pos, 0, 0))
            v_ = F.unfold(v_, kernel_size=(self.split_size, self.split_size * (self.pos * 2 + 1)),
                          stride=self.split_size)
        else:
            # print('----------y axis--------------')
            k_ = F.pad(k_, (0, 0, self.split_size * self.pos, self.split_size * self.pos))
            k_ = F.unfold(k_, kernel_size=(self.split_size * (self.pos * 2 + 1), self.split_size),
                          stride=self.split_size)
            v_ = F.pad(v_, (0, 0, self.split_size * self.pos, self.split_size * self.pos))
            v_ = F.unfold(v_, kernel_size=(self.split_size * (self.pos * 2 + 1), self.split_size),
                          stride=self.split_size)

        # print('3) q k v_ - ', q_.shape, k_.shape, v_.shape)

        k_ = rearrange(k_, 'b (c j) i -> (b i) j c', c=self.dim)
        v_ = rearrange(v_, 'b (c j) i -> (b i) j c', c=self.dim)
        # print('4) q k v_ - ', q_.shape, k_.shape, v_.shape)

        # Divide Embedding into Head
        q, k = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=self.num_heads), (q_, k_))
        q *= self.scale
        # print('5) q k v_ - ', q.shape, k.shape, v_.shape)

        # v에 get_lepe 또는 q에 rel_pos_emb 더하기
        v, lepe = self.get_lepe(v_, self.get_v)
        # print('6) q k v_ - ', q.shape, k.shape, v.shape)

        # Attn 구하기
        sim = einsum('b i d, b j d -> b i j', q, k)
        # print('7) qk - ', sim.shape)
        ...
        out = rearrange(out, '(b h w) (p1 p2) c -> b (h p1) (w p2) c', b=B_, h=(H // self.split_size),
                        w=(W // self.split_size), p1=self.split_size, p2=self.split_size)
        # print('10) out - ', out.shape)
...
class myAttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.branch_num == 2:
            ...
            # print(' --- boundary of x1 and x2 ---')
            x2 = self.attns[1](qkv[:,:,:,C//2:])
            attened_x = torch.cat([x1,x2], dim=2)
            # print('x1 and x2 shape is ', x1.shape, x2.shape)
            # print('attened x is : ', attened_x.shape)
        else:
            # print('branch num is not 2!')
        ...
        # print('10) myAttn output : ', x.shape)
